...
// POST isteği ile form gönderildiğinde çalışacak
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $kullaniciadi = $_POST['kullaniciadi'];
    $parola = $_POST['parola'];

    // Veritabanından kullanıcı sorgulama
    $sorgu = $pdo->prepare("SELECT * FROM users WHERE kullaniciadi = ?");
    $sorgu->execute([$kullaniciadi]);
    $kullanici = $sorgu->fetch(PDO::FETCH_ASSOC);

    if ($kullanici && password_verify($parola, $kullanici['parola'])) {
        // Kullanıcı bulundu ve şifre eşleşti, session'a bilgileri kaydet
        $_SESSION['kullaniciadi'] = $kullanici['kullaniciadi'];
        $_SESSION['adsoyad'] = $kullanici['adsoyad'];
        $_SESSION['organizasyon'] = $kullanici['organizasyon'];
        $_SESSION['kampus'] = $kullanici['kampus'];

        // Kullanıcıyı index.php sayfasına yönlendir
        header("Location: index.php");
        exit;
    } else {
        // Kullanıcı bulunamadı veya şifre eşleşmedi, hata mesajı
                    ...
                    >
                        ...
                        >
                            ...
                            <div class="mx-auto w-full max-w-[440px]">
                                <div class="mb-10">
                                    <h1 class="text-3xl font-extrabold uppercase !leading-snug text-primary md:text-4xl">CENPRO</h1>
                                    <p class="text-base font-bold leading-normal text-white-dark">Kullanıcı adı ve parola ile giriş yapabilirsiniz.</p>
                                </div>
                                ...
                                <form class="space-y-5 dark:text-white" method="POST">
                                    <div>
                                        ...
                                        <div class="relative text-white-dark">
                                            <input id="Username" name="kullaniciadi" type="text" placeholder="Kullanıcı Adı" class="form-input ps-10 placeholder:text-white-dark" />
